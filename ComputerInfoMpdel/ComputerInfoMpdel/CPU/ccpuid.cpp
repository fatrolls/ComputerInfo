...
const CPUIDFIELDDESC CCPUID::CPUFDesc[] = {
	...
	,{CPUF_CMPXCHG16B, 0, "CMPXCHG16B", "CMPXCHG16B instruction."}
	...
	,{CPUF_INVPCID, 0, "INVPCID", "INVPCID instruction."}
	,{CPUF_RTM, 0, "RTM", ""}
...
// 刷新信息.在Info数组中追加一项.
void CCPUID::RefreshInfo_Put(INT32 fid, INT32 fidsub, INT32 CPUInfo[4])
{
...
void CCPUID::RefreshInfo()
{
	...
	if (nmax!=0)
	{
		for(i=0x80000001; i<=nmax; ++i)
		{
			__cpuidex(CPUInfo, i, 0);
			RefreshInfo_Put(i, 0, CPUInfo);
			// fidsub
			if (0x8000001D==i)	// Cache Properties (Function 8000001Dh)
			{
				j=1;
				while(true)
				{
...
void CCPUID::RefreshProperty()
{
	...
	*(INT32*)&_Vendor[0] = dwBuf[1];	// ebx: 前四个字符.
	*(INT32*)&_Vendor[4] = dwBuf[3];	// edx: 中间四个字符.
	*(INT32*)&_Vendor[8] = dwBuf[2];	// ecx: 最后四个字符.
	_Vendor[12] = '\0';
	...
	{
		// Function 80000002h,80000003h,80000004h: Processor Brand String
		GetData((INT32*)&_Brand[0], 0x80000002);	// 前16个字符.
		GetData((INT32*)&_Brand[16], 0x80000003);	// 中间16个字符.
		GetData((INT32*)&_Brand[32], 0x80000004);	// 最后16个字符.
		_Brand[48] = '\0';
	}
	_BrandTrim = &_Brand[0];
...
LPCCPUIDINFO CCPUID::GetInfo(INT32 InfoType, INT32 ECXValue) const
{
	// 顺序搜索.
...
// InfoType: 功能号. 即CPUID指令的eax参数.
// ECXValue: 子功能号. 即CPUID指令的ecx参数.
void CCPUID::GetData(INT32 CPUInfo[4], INT32 InfoType, INT32 ECXValue) const
{
...
// 取得CPUID字段
UINT32 CCPUID::GetField(CPUIDFIELD cpuf) const
{
...
int	CCPUID::simd_avx_level(int* phwavx) const
{
	...
	{
		UINT32 n = GetField(CPUF_XFeatureSupportedMaskLo);	// XCR0: XFeatureSupportedMask register.
		if (6==(n&6))	// XCR0[2:1] = ‘11b’ (XMM state and YMM state are enabled by OS).
		{
			return rt;
		}
