...
//#include "..//stdafx.h"
#include "stdafx.h"
#include "SysVerInfo.h"

#include <tchar.h>
#include <stdio.h>
#include "..\CharaSet\CharaseChange\AscIIToMutlipleCharaset.h"
...
//LPCTSTR CtCharToLPCTSTR(const char* Ctchar)
//{
...
//
//
//LPCWSTR CtCharToLPCWSTR(const char* Ctchar)
//{
//	int num = MultiByteToWideChar(0, 0, Ctchar, -1, nullptr, 0);
//
//	wchar_t *wide = new wchar_t[num];
//	MultiByteToWideChar(0, 0, Ctchar, -1, wide, num);
//	return (LPCWSTR)(LPSTR)wide;
//}
...
void SystemInfo::DetectWindowsVersion()
{
	...
	else // Test for specific product on Windows NT 4.0 SP5 and earlier
	{
		...
		lRet = RegOpenKeyEx(HKEY_LOCAL_MACHINE,
			(LPCWSTR)L"SYSTEM\\CurrentControlSet\\Control\\ProductOptions",
			0, KEY_QUERY_VALUE, &hKey);
		if (lRet != ERROR_SUCCESS)
			return;

		lRet = RegQueryValueEx(hKey, (LPCWSTR)L"ProductType", NULL, NULL,
		...
		if (lstrcmpi((LPCWSTR)L"WINNT", (LPCWSTR)szProductType) == 0)
		{
		...
		if (lstrcmpi((LPCWSTR)L"LANMANNT", (LPCWSTR)szProductType) == 0)
		{
		...
		if (lstrcmpi((LPCWSTR)L"SERVERNT", (LPCWSTR)szProductType) == 0)
		{
			if (m_osvi.dwMajorVersion == 5 && m_osvi.dwMinorVersion == 2)
			{
				m_nWinVersion = WindowsServer2003;
...
void SystemInfo::DetectWindowsServicePack()
{
	...
	if (m_osvi.dwMajorVersion == 4 &&
		lstrcmpi(m_osvi.szCSDVersion, (LPCWSTR)L"Service Pack 6") == 0)
	{
		HKEY hKey;
		LONG lRet;
		...
		lRet = RegOpenKeyEx(HKEY_LOCAL_MACHINE,
			(LPCWSTR)L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Hotfix\\Q246009",
			0, KEY_QUERY_VALUE, &hKey);
		if (lRet == ERROR_SUCCESS)
...
void SystemInfo::GetServicePackInfo1(TCHAR* szServicePack) const
{
	if (szServicePack == NULL) return;

	_tcscpy(szServicePack, CMutilpleCharasetToAscII::CtCharToLPCTSTR(m_szServicePack));
}

void SystemInfo::GetServiceInfoPack(TCHAR* szServicePack)
{
	if (szServicePack == NULL) return;
