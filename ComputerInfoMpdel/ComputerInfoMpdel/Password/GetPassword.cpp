...
wstring getWinError(bool automatic, DWORD code)
{
	bool success = false;
...
bool searchMemory(const PBYTE BaseAddress, const PBYTE addressMaxMin, const PBYTE pattern, PBYTE *PatternAddress, size_t length, bool forward, HANDLE handleProcess)
{
...
bool genericPatternSearch(PBYTE * thePtr, WCHAR *moduleName, BYTE pattern[], ULONG sizeofPattern, LONG offSetToPtr, char *startFunc, bool forward, bool noPtr)
{
	bool result = false;
...
wstring stringOfHex(const BYTE myTab[], DWORD mySize, DWORD length)
{
...
wstring stringOrHex(const BYTE myTab[], DWORD mySize, DWORD length, bool line)
{
	wstring result;
	if (myTab && length > 0)
	{
...
void genericCredsToStream(PGENERIC_PRIMARY_CREDENTIAL mesCreds, bool isDomainFirst, PDWORD pos)
{
	if (mesCreds)
	{
